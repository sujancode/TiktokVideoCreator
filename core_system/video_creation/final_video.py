...
from core_system.utils.cleanup import cleanup
import pathlib
import boto3
...
def make_final_video(
    ...
    opacity = 0.9
    background_clip = (
        VideoFileClip(f"assets/{username}/temp/background.mp4")
        .without_audio()
        .resize(height=H)
    ...
    audio_clips = [AudioFileClip(f"assets/{username}/temp/mp3/{i}.mp3") for i in range(number_of_clips)]
    audio_clips.insert(0, AudioFileClip(f"assets/{username}/temp/mp3/title.mp3"))
    ...
    image_clips.insert(
        0,
        ImageClip(f"assets/{username}/temp/png/title.png")
    ...
    for i in range(0, number_of_clips):
        image_clips.append(
            ImageClip(f"assets/{username}/temp/png/comment_{i}.png")
    ...
    if not exists(f"./results/{username}"):
        print("The results folder didn't exist so I made it")
        os.makedirs(f"./results/{username}")
    ...
    final.write_videofile(
        f"assets/{username}/temp/temp.mp4",
        fps=30,
        audio_codec="aac",
        audio_bitrate="192k",
        verbose=False,
    ...
    ffmpeg_extract_subclip(
        ...
        0,
        final.duration,
        targetname=f"results/{username}/{filename}",
    )


    print("Removing temporary files 🗑")
    cleanups = cleanup(username)
    ...
    return f"results/{username}/{filename}"